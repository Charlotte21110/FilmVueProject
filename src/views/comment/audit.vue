<template>
    <div>
          ...
          <el-col :span="20">
            <div class="grid-content bg-purple">
              <el-select v-model="searchModel.status" clearable placeholder="审核情况">
                <el-option v-for="item in statusInfo" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </div>
          </el-col>
      ...
      <el-card>
        ...
        <el-table :data="auditList" stripe style="width: 100%">
          <el-table-column prop="auditId" label="#" width="150">
          </el-table-column>
          <el-table-column prop="userName" label="用户名" width="200">
          </el-table-column>
          <el-table-column prop="comment" label="评论" width="180" show-overflow-tooltip>
          </el-table-column>
      ...
      <el-dialog @close="clearForm" :title="title" :visible.sync="dialogFormVisible">
        ...
        <el-form :model="auditForm">
          <el-form-item label="审核信息" label-width="80px" prop="ruleAuditInfo">
            <el-input v-model="auditForm.auditInfo" autocomplete="off"></el-input>
          </el-form-item>
          <el-form-item label="审核状态" label-width="80px" prop="ruleStatus">
            <el-switch v-model="auditForm.status" active-color="#13ce66" inactive-color="#ff4949" :active-value="'审核通过'"
              :inactive-value="'审核不通过'">
            </el-switch>
          </el-form-item>
        </el-form>
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="auditComment(auditForm)">确认</el-button>
        </div>
      </el-dialog>
  ...
  import auditApi from '@/api/commentAudit'
  ...
  export default {
    ...
    methods: {
      auditComment(auditForm) {
        auditApi.auditComment(auditForm).then(response => {
          console.log(this.auditForm, 'auditForm');
          this.$message({
      ...
      openEditUI(auditId) {
        ...
        auditApi.getAuditById(auditId).then(response => {
          ...
          this.auditForm.commentId = response.data.commentId;
        });
